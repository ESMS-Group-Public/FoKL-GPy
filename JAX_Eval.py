...
__all__ = ["evaluate_preprocess", "evaluate", "evaluate_basis_jax"]
...
def evaluate(model, inputs=None, betas=None, mtx=None, avgbetas=False, **kwargs):
    """
    ...
    elif model.kernel == model.kernels[1]:  # == 'Bernoulli Polynomials':
        phind = None
        xsm = normputs
    else:
    ...
    #                 else:
    #                     raise ValueError("Kernel must be either 'Cubic Splines' or 'Bernoulli Polynomials'")
    #                 phi *= evaluate_basis_jax(model, coeffs, xsm[i, k])  # multiplies phi(x0)*phi(x1)*etc.
    ...
    aaa = jax.vmap(logD_func, in_axes=(None,0,0,0,None))
    bbb = jax.vmap(
            aaa,
    ...
    X_vec = jax.numpy.prod(ccc(phis, phind, X, mtx.astype(int), betas[:,1:]), axis = 2)
    ...
    #
    # def modellsfunc(X, betas, setnos):
    #     return jax.numpy.transpose(jax.numpy.matmul(X, jax.numpy.transpose(betas[setnos])))

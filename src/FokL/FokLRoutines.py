        ...
        for t in range(len(T) - 1):
            ...
            for i in range(len(y)):
                for j in range(len(y)):
                    if used_inputs[i][j] != 0:
                        if len(inputs1[i]) == 0:
                            inputs1[i] = FokL.normalize(y[j], norms[0, j], norms[1, j])
                        else:
                            inputs1[i] = np.append(inputs1[i], FokL.normalize(y[j], norms[0, j], norms[1, j]), 1)
            # if b.shape[0] > 0:
            ...
            for ii in range(len(y0)):
                if np.amax(used_inputs[ii]) > 1:
                    inputs1[ii] = FokL.reorder(used_inputs[ii], inputs1[ii])
            ...
            for i in range(len(y)):
                for j in range(len(y)):
                    if used_inputs[i][j] != 0:
                        if len(inputs2[i]) == 0:
                            inputs2[i] = FokL.normalize(y[j] + dy1[j] / 2, norms[0, j], norms[1, j])
                        else:
                            inputs2[i] = np.append(inputs2[i], FokL.normalize(y[j] + dy1[j] / 2, norms[0, j], norms[1, j]),
                                                   1)
            # if b.shape[1] > 0:
            #     for k in range(len(y)):
            ...
            for ii in range(len(y0)):
                if np.amax(used_inputs[ii]) > 1:
                    inputs2[ii] = FokL.reorder(used_inputs[ii], inputs2[ii])
            ...
            for i in range(len(y)):
                for j in range(len(y)):
                    if used_inputs[i][j] != 0:
                        if len(inputs3[i]) == 0:
                            inputs3[i] = FokL.normalize(y[j] + dy2[j] / 2, norms[0, j], norms[1, j])
                        else:
                            inputs3[i] = np.append(inputs3[i], FokL.normalize(y[j] + dy2[j] / 2, norms[0, j], norms[1, j]),
                                                   1)
            # if b.shape[1] > 0:
            #     for k in range(len(y)):
            ...
            for ii in range(len(y0)):
                if np.amax(used_inputs[ii]) > 1:
                    inputs3[ii] = FokL.reorder(used_inputs[ii], inputs3[ii])
            ...
            for i in range(len(y)):
                for j in range(len(y)):
                    if used_inputs[i][j] != 0:
                        if len(inputs4[i]) == 0:
                            inputs4[i] = FokL.normalize(y[j] + dy3[j], norms[0, j], norms[1, j])
                        else:
                            inputs4[i] = np.append(inputs4[i], FokL.normalize(y[j] + dy3[j], norms[0, j], norms[1, j]), 1)
            # if b.shape[1] > 0:
            #     for k in range(len(y)):
            #         inputs4[k] = np.append(inputs4[k], othinputs[k])
            for ii in range(len(y0)):
                if np.amax(used_inputs[ii]) > 1:
                    inputs4[ii] = FokL.reorder(used_inputs[ii], inputs4[ii])

...
| ```m.fokl_expr```      | pyo.Expression | FoKL model function (i.e., $\overline{\beta}$ draw in $\beta_0 +\sum_{j=1:m} \sum_{i=1:n} \beta_{ij} (B_i (x_j ))+\sum_{j=1:m-1} \sum_{k=j+1:m} \sum_{i=1:n} (\beta_{ijk} \cdot B_i (x_j , x_k ))+\dots$ where $B_i=$ basis function) |

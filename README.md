...
| ```m.fokl_expr```      | pyo.Expression | FoKL model function (i.e., $\overline{\beta}$ draw in $\beta_0 +\sum^m_{j=1} \sum^n_{i=1} \beta_{ij} \scriptB_i (x_j )+\sum_{j=1:m-1} \sum_{k=j+1:m} \sum_{i=1:n} \beta_{ijk} B_i (x_j , x_k )+\dots$ where $i = $scenario) |
